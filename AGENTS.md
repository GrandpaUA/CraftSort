...
Includes a "new recipe" indicator (blue dot) and filter tab.
...
### UI layout
- Two separate containers anchored to panel top-left, offset (-9, -200)
...
InventoryGui+RecipeDataPair (private struct):
- Recipe, ItemData, InterfaceElement (GameObject), CanCraft
- Accessed via reflection (PropertyInfo or FieldInfo, cached)
...
- LogOutput.log line: [Info : BepInEx] Loading [CraftSort 1.1.0]
